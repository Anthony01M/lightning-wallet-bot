...
export default class StringMenuBuilder<Excluded extends (keyof StringMenuBuilder)[] = [], Arguments extends any[] = []> {
	protected listener: (ctx: Context<StringSelectMenuInteraction>, ...args: Arguments) => any | Promise<any> = () => undefined
	protected m_name = ''

	public setName(name: string): Omit<StringMenuBuilder<[...Excluded, 'setName'], Arguments>, 'setName' | Excluded[number]> {
		this.m_name = name
		return this as any
	}

	public listen<Args extends any[]>(callback: (ctx: Context<StringSelectMenuInteraction>, ...args: Args) => any | Promise<any>): Omit<StringMenuBuilder<[...Excluded, 'listen'], Args>, 'listen' | Excluded[number]> {
