...
export default class RoleMenuBuilder<Excluded extends (keyof RoleMenuBuilder)[] = [], Arguments extends any[] = []> {
	protected listener: (ctx: Context<RoleSelectMenuInteraction>, ...args: Arguments) => any | Promise<any> = () => undefined
	protected m_name = ''

	public setName(name: string): Omit<RoleMenuBuilder<[...Excluded, 'setName'], Arguments>, 'setName' | Excluded[number]> {
		this.m_name = name
		return this as any
	}

	public listen<Args extends any[]>(callback: (ctx: Context<RoleSelectMenuInteraction>, ...args: Args) => any | Promise<any>): Omit<RoleMenuBuilder<[...Excluded, 'listen'], Args>, 'listen' | Excluded[number]> {
